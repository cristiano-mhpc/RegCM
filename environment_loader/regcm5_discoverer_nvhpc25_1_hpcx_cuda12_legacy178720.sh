#!/usr/bin/env bash
# Reconstructed legacy Discoverer+ loader from job 178720 logs.
# Purpose: reproduce the pre-fix runtime without modifying the working loader.
...
_regcm_cuda_stubs_policy="${REGCM_CUDA_STUBS:-auto}"
...
[ -n "${REGCM_DISCOVERER_HDF5:-}" ] && export HDF5="$REGCM_DISCOVERER_HDF5"
[ -n "${REGCM_DISCOVERER_NETCDF_C:-}" ] && export NETCDF="$REGCM_DISCOVERER_NETCDF_C"
...
if command -v nc-config >/dev/null 2>&1; then
  ...
  [ -n "$_regcm_netcdf_c_prefix" ] && export NETCDF="$_regcm_netcdf_c_prefix"
...
[ -n "${_regcm_extra_ldflags:-}" ] && export LDFLAGS="$_regcm_extra_ldflags ${LDFLAGS:-}"
...
[ "${_regcm_restore_nounset:-0}" = 1 ] && set -u
[ "${_regcm_restore_errexit:-0}" = 1 ] && set -e
if [ "${_regcm_loader_failed:-0}" = 1 ]; then
  return 1 2>/dev/null || exit 1
fi

unset _regcm_loader_failed _regcm_tool _regcm_prefix _regcm_cuda_stubs_policy _regcm_cuda_stubs_status _regcm_gpu_driver_present _regcm_use_cuda_stubs _regcm_cuda_target_lib _regcm_extra_ldflags _regcm_nvhpc_compiler_lib _regcm_nvhpc_math _regcm_io_stack_status _regcm_netcdf_c_prefix _regcm_netcdf_c_libdir _regcm_netcdf_f_prefix _regcm_pnetcdf_prefix _regcm_restore_errexit _regcm_restore_nounset
