module mod_clm_soilhydrology
  ...
  subroutine SurfaceRunoff (lbc, ubc, num_hydrologyc, &
    ...
    real(rk8), pointer, contiguous :: wtfact(:)
    ...
    real(rk8), pointer, contiguous :: eff_porosity(:,:)
    !fractional impermeability (-)
    real(rk8), pointer, contiguous :: fracice(:,:)
#if (defined VICHYDRO)
    ...
    real(rk8), pointer, contiguous :: moist(:,:)
    !ice len in each VIC layers(ice, mm)
    real(rk8), pointer, contiguous :: ice(:,:)
    !maximum infiltration capacity in VIC (mm)
    real(rk8), pointer, contiguous :: max_infil(:)
    !column average soil moisture in top VIC layers (mm)
    real(rk8), pointer, contiguous :: i_0(:)
#endif
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      top_ice(c)     = max(0.0_rk8,top_ice(c))
      max_infil(c)   = (1.0_rk8+b_infil(c)) * top_max_moist(c)
      ex(c)          = b_infil(c) / (1.0_rk8 + b_infil(c))
      A(c)           = 1.0_rk8 - (1.0_rk8 - top_moist(c) / top_max_moist(c))**ex(c)
      i_0(c)         = max_infil(c) * (1.0_rk8 - (1.0_rk8 - A(c))**(1.0_rk8/b_infil(c)))
    ...
    do concurrent ( fc = 1:num_urbanc )
      ...
      if ( ctype(c) == icol_roof .or. &
        ...
        if (snl(c) < 0) then
          qflx_surf(c) = max(0.0_rk8,qflx_top_soil(c))
        else
          xs(c) = max(0.0_rk8, &
          ...
          if (xs(c) > 0.0_rk8 ) then
            ...
            h2osoi_liq(c,1) = max(0.0_rk8,h2osoi_liq(c,1)+ &
  ...
  subroutine Infiltration(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
    ...
    integer(ik4), pointer, contiguous :: clandunit(:)  ! column's landunit
    integer(ik4), pointer, contiguous :: ltype(:)      ! landunit type
    real(rk8), pointer, contiguous :: h2osfc_thresh(:)  ! level at which h2osfc "percolates"
    ! fraction of ground covered by snow (0 to 1)
    real(rk8), pointer, contiguous :: frac_sno(:)
    ! ground surface evaporation rate (mm H2O/s) [+]
    real(rk8), pointer, contiguous :: qflx_evap_soi(:)
    real(rk8), pointer, contiguous :: qflx_h2osfc_surf(:) ! surface water runoff (mm/s)
    real(rk8), pointer, contiguous :: h2osfc(:)           ! surface water (mm)
    ! fraction of ground covered by surface water (0 to 1)
    real(rk8), pointer, contiguous :: frac_h2osfc(:)
    ! fraction of ground covered by surface water (0 to 1)
    real(rk8), pointer, contiguous :: frac_h2osfc_temp(:)
...
#if (defined VICHYDRO)
    real(rk8), pointer, contiguous :: b_infil(:)   !VIC b infiltration parameter
    !maximum soil moisture (ice + liq, mm)
    real(rk8), pointer, contiguous :: max_moist(:,:)
    !soil moisture in each VIC layers (liq, mm)
    real(rk8), pointer, contiguous :: moist(:,:)
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      if ( ltype(l) == istsoil .or. ltype(l) == istcrop ) then
        ...
        qflx_in_soil(c) = (1.0_rk8 - frac_h2osfc(c)) * &
        ...
        top_icefrac = min(1.0_rk8,top_ice(c)/top_max_moist(c))
        if ( qflx_in_soil(c) <= 0.0_rk8 ) then
          rsurf_vic = 0.0_rk8
        else if ( max_infil(c) <= 0.0_rk8 ) then
          ...
          basis = 1.0_rk8 - (i_0(c) + qflx_in_soil(c)*dtsrf)/max_infil(c)
          rsurf_vic = (qflx_in_soil(c)*dtsrf - &
                  top_max_moist(c) + top_moist(c)    &
                + top_max_moist(c) * basis**(1.0_rk8 + b_infil(c)))/dtsrf
        end if
        rsurf_vic = max(0.0_rk8, rsurf_vic)
        qinmax = (1.0_rk8 - fsat(c)) * &
                10.0_rk8**(-e_ice*top_icefrac)*(qflx_in_soil(c) - rsurf_vic)
#else
        qinmax = (1.0_rk8 - fsat(c)) * &
                minval(10.0_rk8**(-e_ice*(icefrac(c,1:3)))*hksat(c,1:3))
#endif
        qflx_infl_excess(c) = max(0.0_rk8,qflx_in_soil(c) - &
        ...
        if ( h2osfc(c) >= h2osfc_thresh(c) .and. &
          ...
          qflx_h2osfc_surf(c) = 0.0_rk8
        end if

        ! cutoff lower limit
        if ( qflx_h2osfc_surf(c) < 1.0e-8_rk8 ) qflx_h2osfc_surf(c) = 0.0_rk8
        ...
        if ( h2osfc(c) < 0.0_rk8 ) then
          qflx_infl(c) = qflx_infl(c) + h2osfc(c)/dtsrf
          h2osfc(c) = 0.0_rk8
          qflx_h2osfc_drain(c) = 0.0_rk8
        else
          qflx_h2osfc_drain(c) = min(frac_h2osfc(c)*qinmax,h2osfc(c)/dtsrf)
        end if

        if ( h2osfcflag == 0 ) then
          qflx_h2osfc_drain(c) = max(0.0_rk8,h2osfc(c)/dtsrf) !ensure no h2osfc
        end if
  ...
  subroutine SoilWater(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
    ...
    real(rk8), pointer, contiguous :: h2osoi_ice(:,:)  ! ice water (kg/m2)
    ! true=>do computations on this pft (see reweightMod for details)
    logical, pointer, contiguous :: pactive(:)
    integer(ik4), pointer, contiguous :: ctype(:)  ! column type index
    integer(ik4), pointer, contiguous :: npfts(:)  ! column's number of pfts - ADD
    real(rk8), pointer, contiguous :: pwtcol(:)     ! weight relative to column for each pft
    ...
    real(rk8), pointer, contiguous :: qflx_tran_veg_pft(:)
    ! vegetation transpiration (mm H2O/s) (+ = to atm)
    real(rk8), pointer, contiguous :: qflx_tran_veg_col(:)
    ! effective porosity = porosity - vol_ice
    real(rk8), pointer, contiguous :: eff_porosity(:,:)
    ! volumetric soil water at saturation (porosity)
    real(rk8), pointer, contiguous :: watsat(:,:)
    ...
    real(rk8), pointer, contiguous :: rootr_pft(:,:)
    ...
    real(rk8), pointer, contiguous :: hkdepth(:)  ! decay factor (m)
    real(rk8), pointer, contiguous :: zwt(:)      ! water table depth (m)
    real(rk8), pointer, contiguous :: zi(:,:)     ! interface level below a "z" level (m)

    real(rk8), pointer, contiguous :: h2osoi_liq(:,:) ! liquid water (kg/m2)
    ! effective fraction of roots in each soil layer
    real(rk8), pointer, contiguous :: rootr_col(:,:)
        ...
        do j = nlevsoi,nlevgrnd
          if ( zwt(c) <= zi(c,j) ) then
            smp1 = hfus*(tfrz-t_soisno(c,j)) / &
                    (grav*t_soisno(c,j)) * 1000.0_rk8  !(mm)
            !smp1 = max(0.0_rk8,smp1)
            smp1 = max(sucsat(c,nlevsoi),smp1)
            vwc_zwt(c) = watsat(c,nlevsoi) * &
                    (smp1/sucsat(c,nlevsoi))**(-1.0_rk8/bsw(c,nlevsoi))
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      if ( jwt(c) == nlevsoi ) then
        ...
        vol_eq(c,j+1) = -sucsat(c,j)*watsat(c,j) / &
                (1.0_rk8-1.0_rk8/bsw(c,j))/(zwtmm(c)-zimm(c,j))*(tempi-temp0)
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      if ( j > jwt(c) ) then !water table is in soil column
        ...
        qout(c,j+1)   =  0.0_rk8  ! zero-flow bottom boundary condition
        dqodw1(c,j+1) =  0.0_rk8  ! zero-flow bottom boundary condition
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      if ( jwt(c) < nlevsoi ) then
        ...
        if ( jwt(c) == 0 ) then
          qcharge(c) = -ka * (wh_zwt-wh)  /((zwt(c)+1.e-3_rk8)*1000.0_rk8)
        else
          ! qcharge(c) = -ka * (wh_zwt-wh)/((zwt(c)-z(c,jwt(c)))*1000.0_rk8)
          !scs: 1/2, assuming flux is at zwt interface, saturation
          ! deeper than zwt
          qcharge(c) = -ka*(wh_zwt-wh)/((zwt(c)-z(c,jwt(c)))*1000.0_rk8*2.0_rk8)
        end if
  ...
  subroutine Drainage(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
    ...
    real(rk8), pointer, contiguous :: h2osfc(:)         ! surface water (mm)
    real(rk8), pointer, contiguous :: frac_h2osfc(:)    !
    real(rk8), pointer, contiguous :: topo_ndx(:)       ! topographic index
    real(rk8), pointer, contiguous :: topo_slope(:)     ! topographic slope
    real(rk8), pointer, contiguous :: frost_table(:)    ! frost table depth (m)
    real(rk8), pointer, contiguous :: zwt_perched(:)    ! perched water table depth (m)
    ...
    real(rk8), pointer, contiguous :: qcharge(:)    ! aquifer recharge rate (mm/s)
#if (defined VICHYDRO)
    real(rk8), pointer, contiguous :: moist(:,:)    !soil layer moisture (mm)
    real(rk8), pointer, contiguous :: ice(:,:)      !soil layer moisture (mm)
    !fracton of Dsmax where non-linear baseflow begins
    real(rk8), pointer, contiguous :: Ds(:)
    real(rk8), pointer, contiguous :: Dsmax(:)      !max. velocity of baseflow (mm/day)
    !fraction of maximum soil moisutre where non-liear base flow occurs
    real(rk8), pointer, contiguous :: Wsvic(:)
    real(rk8), pointer, contiguous :: c_param(:)     !baseflow exponent (Qb)
    real(rk8), pointer, contiguous :: max_moist(:,:) !maximum soil moisture (ice + liq)
    real(rk8), pointer, contiguous :: depth(:,:)     !VIC soil depth
    real(rk8), pointer, contiguous :: hk_l(:,:)      !hydraulic conductivity (mm/s)
    ...
    real(rk8), pointer, contiguous :: qflx_drain(:)
    ...
    real(rk8), pointer, contiguous :: eflx_impsoil(:)
    ! soil saturation excess [mm h2o/s]
    real(rk8), pointer, contiguous :: qflx_rsub_sat(:)
...
!-----------------------------------------------------------------------
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      rsub_bot(c)      = 0.0_rk8
      qflx_rsub_sat(c) = 0.0_rk8
      rsub_top(c)      = 0.0_rk8
      fracice_rsub(c)  = 0.0_rk8
    end do
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      if ( jwt(c) == nlevsoi ) then
        ...
        zwt(c) = zwt(c) - (qcharge(c)  * dtsrf)/1000.0_rk8/rous
      else
        ...
        if ( qcharge_tot > 0.0_rk8 ) then !rising water table
          ...
          do j = jwt(c)+1, 1, -1
            ...
            qcharge_layer = max(qcharge_layer,0.0_rk8)

            if ( s_y > 0.0_rk8 ) zwt(c) = zwt(c) - qcharge_layer/s_y/1000.0_rk8
          ...
          do j = jwt(c)+1, nlevsoi
            ...
            qcharge_layer = min(qcharge_layer,0.0_rk8)
            qcharge_tot = qcharge_tot - qcharge_layer

            if ( qcharge_tot >= 0.0_rk8 ) then
              zwt(c) = zwt(c) - qcharge_layer/s_y/1000.0_rk8
              exit
            else
              zwt(c) = zi(c,j)
            end if
          end do
          if ( qcharge_tot > 0.0_rk8 ) zwt(c) = zwt(c) - qcharge_tot/1000.0_rk8/rous
        end if
    ...
    do fc = 1, num_hydrologyc
      ...
      q_perch_max = 1.e-5_rk8 * sin(topo_slope(c) * (rpi/180.0_rk8))
      ...
      if ( zwt(c) < frost_table(c) .and. &
        ...
        if ( wtsub > 0.0_rk8 ) q_perch = q_perch/wtsub
        ...
        do k = jwt(c)+1, k_frz
          ...
          if ( rsub_top_tot >= 0.0_rk8 ) then
            zwt(c) = zwt(c) - rsub_top_layer/eff_porosity(c,k)/1000.0_rk8
            exit
          else
            zwt(c) = zi(c,k)
          end if
        ...
        if ( k_frz > k_perch ) then
          ...
          zwt_perched(c)=max(0.0_rk8,m*sat_lev+b)
          ...
          if ( origflag == 1 ) qflx_drain_perched(c) = 0.0_rk8
          ...
          do k = k_perch+1, k_frz
            ...
            if ( rsub_top_tot >= 0.0_rk8 ) then
              zwt_perched(c) = zwt_perched(c) - &
                      rsub_top_layer/eff_porosity(c,k)/1000.0_rk8
              exit
            else
...
#else
          imped = 10.0_rk8**(-e_ice*(icefracsum/dzsum))
          rsub_top_max = 10.0_rk8 * sin((rpi/180.0_rk8) * topo_slope(c))
#endif
        end if
#if (defined VICHYDRO)
        ...
        if ( jwt(c) == nlevsoi ) then
          ...
          zwt(c) = zwt(c) + (rsub_top(c) * dtsrf)/1000.0_rk8/rous
          h2osoi_liq(c,nlevsoi) = h2osoi_liq(c,nlevsoi) + &
                  max(0.0_rk8,(wa(c)-5000.0_rk8))
          wa(c)  = min(wa(c), 5000.0_rk8)
        else
          ! water table within soil layers 1-9
          ...
          if ( rsub_top_tot > 0.0_rk8 ) then !rising water table
            ...
            do j = jwt(c)+1, nlevsoi
              ...
              if ( rsub_top_tot >= 0.0_rk8 ) then
                zwt(c) = zwt(c) - rsub_top_layer/s_y/1000.0_rk8
                exit
              else
                zwt(c) = zi(c,j)
              end if
            ...
            zwt(c) = zwt(c) - rsub_top_tot/1000.0_rk8/rous
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      xs1(c) = max(max(h2osoi_liq(c,1),0.0_rk8) - &
                   max(0.0_rk8,(pondmx+watsat(c,1)*dzmm(c,1) - &
                       h2osoi_ice(c,1))),0.0_rk8)
      h2osoi_liq(c,1) = min(max(0.0_rk8,pondmx+watsat(c,1)*dzmm(c,1) - &
      ...
      ! add in ice check
      xs1(c) = max(max(h2osoi_ice(c,1),0.0_rk8) - &
                   max(0.0_rk8,(pondmx+watsat(c,1)*dzmm(c,1) - &
                   h2osoi_liq(c,1))),0.0_rk8)
      h2osoi_ice(c,1) = min(max(0.0_rk8,pondmx+watsat(c,1)*dzmm(c,1) - &
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      do j = 1, nlevsoi-1
        if ( h2osoi_liq(c,j) < watmin ) then
          ...
          if ( j == jwt(c) ) then
            zwt(c) = zwt(c) + xs(c)/eff_porosity(c,j)/1000.0_rk8
          endif
        else
          xs(c) = 0.0_rk8
        end if
    ...
    do concurrent ( fc = 1:num_hydrologyc )
      ...
      if ( snl(c)+1 >= 1 ) then
        ...
        h2osoi_liq(c,1) = h2osoi_liq(c,1) + &
                (1.0_rk8 - frac_h2osfc(c))*qflx_dew_grnd(c) * dtsrf
        h2osoi_ice(c,1) = h2osoi_ice(c,1) + &
                (1.0_rk8 - frac_h2osfc(c))*qflx_dew_snow(c) * dtsrf
        if (qflx_sub_snow(c)*dtsrf > h2osoi_ice(c,1)) then
          qflx_sub_snow(c) = h2osoi_ice(c,1)/dtsrf
          h2osoi_ice(c,1) = 0.0_rk8
        else
          h2osoi_ice(c,1) = h2osoi_ice(c,1) - &
                  (1.0_rk8 - frac_h2osfc(c)) * qflx_sub_snow(c) * dtsrf
        end if
      end if
    end do
