module mod_clm_driver
  ...
  subroutine clm_drv(doalb,nextsw_cday,declinp1,declin,rstwr,nlend,nlomon,rdate)
    ...
    do concurrent ( g = begg:endg )
      ...
      clm3%g%gwf%qflx_ice_dynbal(g) = 0._rk8
      clm3%g%gws%gc_ice2(g)         = 0._rk8
      clm3%g%gws%gc_ice1(g)         = 0._rk8
      clm3%g%gef%eflx_dynbal(g)     = 0._rk8
      clm3%g%ges%gc_heat2(g)        = 0._rk8
      clm3%g%ges%gc_heat1(g)        = 0._rk8
    end do
    ...
    do concurrent ( c = begc:endc )
      ...
      if ( itypelun(l) == isturb ) then
        ...
        cptr%cps%frac_sno(c) = min( cptr%cps%snow_depth(c)/0.05_rk8, 1._rk8)
      end if
    end do
...
#if (defined CNDV)
    if ( is_end_curr_year( ) .and.  rcmtimer%integrating( ) )  then
      call histCNDV()
      if (myid == italk) then
