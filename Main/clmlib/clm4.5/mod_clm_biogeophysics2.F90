  ...
  subroutine Biogeophysics2 (lbl, ubl, lbc, ubc, lbp, ubp, &
    ...
    do concurrent ( fc = 1:num_nolakec )
      ...
      if ( j < 0) then
        ...
        t_grnd0(c) = (1.0_rk8 - frac_h2osfc(c)) * tssbef(c,1) + &
          frac_h2osfc(c) * t_h2osfc_bef(c)
      end if
    ...
    do concurrent ( fp = 1:num_nolakep )
      ...
      if (ltype(l) == isturb) then
        ...
        qflx_ev_soil(p) = 0._rk8
        qflx_ev_h2osfc(p) = 0._rk8
      else
    ...
    do concurrent ( fp = 1:num_nolakep )
      ...
      if ( ltype(l) /= isturb ) then
        ...
        eflx_soil_grnd(p) = ((1._rk8- frac_sno_eff(c))*sabg_soil(p) + &
          frac_sno_eff(c)*sabg_snow(p)) + dlrad(p) + &
               real(1-frac_veg_nosno(p),rk8)*emg(c)*forc_lwrad(g) - &
               emg(c)*sb*lw_grnd - emg(c)*sb*t_grnd0(c)**3*(4._rk8*tinc(c)) - &
        ...
        eflx_lwrad_del(p) = 4._rk8*emg(c)*sb*t_grnd0(c)**3*tinc(c)
    ...
    do concurrent ( fp = 1:num_nolakep )
      ...
      if ( ltype(l) /= isturb ) then
        ...
        eflx_lwrad_out(p) = ulrad(p) &
               + (1.0_rk8-frac_veg_nosno(p))*(1.0_rk8-emg(c))*forc_lwrad(g) &
               + (1.0_rk8-frac_veg_nosno(p))*emg(c)*sb*lw_grnd &
               + 4._rk8*emg(c)*sb*t_grnd0(c)**3*tinc(c)
